...
struct qpnp_lpg_channel {
	...
	bool				lut_written;
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
	long				current_period_ns;
	long				current_duty_ns;
#else /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
static void __qpnp_lpg_calc_pwm_period(long period_ns,
			struct lpg_pwm_config *pwm_config)
#else /* CONFIG_LEDS_SHARP */
...
#endif /* CONFIG_LEDS_SHARP */
{
	...
	int tmp1, tmp2;
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
	long clk_period_ns = 0, pwm_clk_period_ns;
#else /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
	pr_debug("PWM setting for period_ns %ld: pwm_clk = %dHZ, prediv = %d, exponent = %d, pwm_size = %d\n",
			period_ns, pwm_config->pwm_clk, pwm_config->prediv,
			pwm_config->clk_exp, pwm_config->pwm_size);
	pr_debug("Actual period: %ldns\n", (long)pwm_config->best_period_ns);
#else /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
static void __qpnp_lpg_calc_pwm_duty(long period_ns, long duty_ns,
			struct lpg_pwm_config *pwm_config)
{
	u16 pwm_value, max_pwm_value;

	if ((1 << pwm_config->pwm_size) > (LONG_MAX / duty_ns)) {
		...
		struct pwm_device *pwm, int duty_ns, int period_ns)
{
	...
	int rc = 0;
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
	long duty_ns_long = duty_ns;
	long period_ns_long = period_ns;
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
		__qpnp_lpg_calc_pwm_period(period_ns_long, &lpg->pwm_config);
#else /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
		__qpnp_lpg_calc_pwm_duty(period_ns_long, duty_ns_long, &lpg->pwm_config);
#else /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
		seq_printf(s, "  Requested period: %dns, best period = %ldns\n",
				pwm_get_period(pwm), (long)cfg->best_period_ns);
#else /* CONFIG_LEDS_SHARP */
