						...
						ktime_t t)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00028 */
	int fps_low_base = MDSS_BASE_FPS_DEFAULT;
#endif /* CONFIG_SHARP_DISPLAY */
	...
	fps_low_base = mdp5_data->fpslow_base;
	fps_low_base = (fps_low_base > 0) ? fps_low_base : MDSS_BASE_FPS_DEFAULT;
	if ((mdss_panel_get_framerate(&(ctl->panel_data->panel_info),
		FPS_RESOLUTION_DEFAULT) / fps_low_base) > mdp5_data->fpslow_count) {
		pr_debug("%s: return....\n", __func__);
		return;
	}
...
static int mdss_fb_set_dfps_pause(struct msm_fb_data_type *mfd, int status)
{
	...
	if (status) {
		data.fps = MDSS_BASE_FPS_60;
		ret = mdss_mdp_dfps_update_params(mfd, pdata, &data);
		if (ret) {
			pr_err("failed to set dfps params\n");
			return ret;
		}
