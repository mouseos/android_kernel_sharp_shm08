...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00026 */
#define MDSS_FB_BL_MAX_LEVEL	100000
#define MDSS_FB_BL_CURR_MAX_UA	100000
...
static void mdss_fb_set_mdp_sync_pt_threshold(struct msm_fb_data_type *mfd,
		int type);
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00026 */
static void mdss_fb_curr_boost_work(struct work_struct *work);
static int mdss_fb_set_max(struct mdss_panel_data *pdata,
					struct msm_fb_data_type *mfd);
static int mdss_fb_restore_current_no_sync(struct mdss_panel_data *pdata,
					struct msm_fb_data_type *mfd);
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00026 */
static ssize_t mdss_fb_curr_boost_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf,PAGE_SIZE,"req:%d,boosted:%d",
			mfd->curr_boost_req, mfd->curr_boosted);
}

static ssize_t mdss_fb_curr_boost_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cancel_delayed_work_sync(&mfd->curr_boost_work);
	mfd->curr_boost_req = (bool)data;
	queue_delayed_work(mfd->ordered_workqueue, &mfd->curr_boost_work, 0);
	return count;
}

/* sysfs attributes exported by curr_boost */
static DEVICE_ATTR(curr_boost, 0664,
		mdss_fb_curr_boost_show, mdss_fb_curr_boost_store);

static struct attribute *boost_attrs[] = {
	&dev_attr_curr_boost.attr,
	NULL
};
static const struct attribute_group mdss_fb_boost_attrs = {
	.attrs = boost_attrs
};
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00026 */
	if (mfd->index == 0) {
		rc = sysfs_create_group(&backlight_led.dev->kobj, &mdss_fb_boost_attrs);

		mutex_init(&mfd->curr_boost_lock);
		mfd->ordered_workqueue =
			alloc_ordered_workqueue("curr_boost_workqueue", 0);
		if (!mfd->ordered_workqueue) {
			rc = -ENOMEM;
			pr_err("%s: alloc_ordered_workqueue failed\n", __func__);
			return rc;
		}
		INIT_DELAYED_WORK(&mfd->curr_boost_work, mdss_fb_curr_boost_work);
	}
#endif /* CONFIG_SHARP_DISPLAY */
	return rc;
}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00026 */
	if (mfd->index == 0) {
		sysfs_remove_group(&backlight_led.dev->kobj,
				&mdss_fb_boost_attrs);

		cancel_delayed_work_sync(&mfd->curr_boost_work);
		destroy_workqueue(mfd->ordered_workqueue);
		mutex_destroy(&mfd->curr_boost_lock);
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		if (mfd->bl_level_scaled == temp) {
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00026 */
			mutex_lock(&mfd->curr_boost_lock);
			mfd->last_level = temp;
			if ((bkl_on != mfd->prev_state) && mfd->curr_boosted) {
				pr_debug("backlight off to curr_boosted false\n");
				mdss_fb_restore_current_no_sync(pdata, mfd);
				mfd->curr_boosted   = false;
				mfd->curr_boost_req = false;
			}
			...
			mfd->prev_state = bkl_on;
			mutex_unlock(&mfd->curr_boost_lock);
	...
	unsigned long *argp, struct file *file)
{
	...
	mdp5_data = mfd_to_mdp5_data(mfd);
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_00027 */
...
static void mdss_fb_curr_boost_work(struct work_struct *work)
{
	...
	mutex_lock(&mfd->curr_boost_lock);
	if(mfd->curr_boost_req == true) {
		mdss_fb_set_max(pdata, mfd);
		mfd->curr_boosted = true;
		mfd->curr_boost_req = false;
		queue_delayed_work(mfd->ordered_workqueue,
					&mfd->curr_boost_work,
...
//PWM set Duty to MAX
	...
	pr_debug("%s: in\n", __func__);
	ret = pwm_config(ctrl_pdata->pwm_bl,
				MDSS_FB_BL_MAX_LEVEL,
				MDSS_FB_BL_CURR_MAX_UA);
	if (ret) {
		pr_err("%s: pwm_config() failed err=%d.\n",
				__func__, ret);
	}
	ret = pwm_enable(ctrl_pdata->pwm_bl);
...
//PWM return Duty
	pr_debug("%s: in\n", __func__);
	pdata->set_backlight(pdata, mfd->last_level);
