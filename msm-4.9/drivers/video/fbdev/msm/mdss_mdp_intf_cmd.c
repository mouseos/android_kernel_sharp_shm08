 ...
 *
 * So it is responsibility of the caller of this function to only call to
 * disable autorefresh if no hw transaction is on-going (wait for ping pong)
 * and if the listening for the external TE is disabled in the tear check
 * logic (this to prevent any race conditions with the hw), as mentioned
 * in the above steps.
 */
