...
#ifdef CONFIG_BATTERY_SHARP
#define DEFAULT_CPUFREQ_SILVER_SOC_EMPTY_CLOCK_LIMIT 	1036800	/* silver(CPU0~CPU3): */
#define DEFAULT_CPUFREQ_GOLD_SOC_EMPTY_CLOCK_LIMIT 		1036800	/* gold(CPU4~CPU7): */
static DEFINE_PER_CPU(unsigned int, limit_freq) = UINT_MAX;
static DEFINE_PER_CPU(unsigned int, pre_limit_freq) = UINT_MAX;
void cpu_clock_limit_lock(struct qpnp_qg *chip)
{
	int cpu;

	qg_dbg(chip, QG_DEBUG_STATUS, "Entered\n");

	for_each_possible_cpu(cpu) {
		if (cpu <= 3) {
			pr_debug("CPU%d limit_freq=%u <-- silver empty_soc freq:%d\n",
				cpu, per_cpu(limit_freq, cpu), chip->dt.empty_soc_silver_freq_khz);
			per_cpu(limit_freq, cpu) = chip->dt.empty_soc_silver_freq_khz;
		}
		if (cpu >= 4) {
			pr_debug("CPU%d limit_freq=%u <-- gold empty_soc freq:%d\n",
				cpu, per_cpu(limit_freq, cpu), chip->dt.empty_soc_gold_freq_khz);
...
void cpu_clock_limit_unlock(struct qpnp_qg *chip)
{
	int cpu;
...
#ifdef CONFIG_BATTERY_SHARP
	chip->stored_learned_capacity = true;
#endif /* CONFIG_BATTERY_SHARP */
			       ...
			       const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STORED_LEARNED_CAPACITY:
		chip->stored_learned_capacity = (pval->intval);
		break;
#endif /* CONFIG_BATTERY_SHARP */
	default:
		break;
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_FULL_DESIGN:
		...
		break;
	case POWER_SUPPLY_PROP_STORED_LEARNED_CAPACITY:
		pval->intval = chip->stored_learned_capacity;
		break;
#endif /* CONFIG_BATTERY_SHARP */
	default:
...
static int limit_lock_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	for_each_possible_cpu(cpu) {
		if (per_cpu(limit_freq, cpu) != per_cpu(pre_limit_freq, cpu)) {
			...
			pr_debug("CPU%d previous limit_freq=%u\n", cpu, per_cpu(pre_limit_freq, cpu));
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	{
		qg_dbg(chip, QG_DEBUG_STATUS, "cpufreq_register_notifier succeeded.\n");
	}
#endif /* CONFIG_BATTERY_SHARP */
