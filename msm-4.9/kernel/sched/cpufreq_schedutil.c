...
#ifdef CONFIG_SHARP_PNP_CLOCK
static unsigned int sh_limit_freq_silver = 1689600;
module_param_named(
	sh_limit_freq_silver, sh_limit_freq_silver, int, S_IRUGO | S_IWUSR | S_IWGRP
);

static unsigned int sh_limit_freq_gold = 1689600;
module_param_named(
...
static bool sh_update_policy_try = false;
#endif /* CONFIG_SHARP_PNP_CLOCK */
...
{
	policy->max = max(per_cpu(sh_load_limit_freq, policy->cpu), per_cpu(sh_fps_limit_freq, policy->cpu));
	sh_cpufreq_frequency_table_target(policy, &policy->max, CPUFREQ_RELATION_L);
...
{
	unsigned int release_limit_freq;
	int cpu, scale_cpu;
	struct cpufreq_policy *policy;
	...
	release_limit_freq = max(per_cpu(sh_load_limit_freq, curr_cpu), per_cpu(sh_fps_limit_freq, curr_cpu));
	...
	if (release_limit_freq != max(per_cpu(sh_load_limit_freq, curr_cpu), per_cpu(sh_fps_limit_freq, curr_cpu)))
		sh_update_policy_try = true;
}
#endif /* CONFIG_SHARP_PNP_CLOCK */
...
#ifdef CONFIG_SHARP_PNP_CLOCK
	if (sh_update_policy_try) {
		sh_update_policy_try = false;
